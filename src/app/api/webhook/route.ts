...
function verifySignatureWithSDK(body: string, signature: string): boolean {
  ...
  export async function POST(req: NextRequest) { 
    ...
    if (!signature || !apiKey) { 
      ...
      let payload: unknown; 
      try { 
        payload = JSON.parse(body) as Record<string, unknown>; } catch { return NextResponse.json({ error: "Invalid JSON" }, { status: 400 }); 
      } 
    ...
    await inngest.send({ 
      ...
      data: { meetingId: updatedMeeting.id, 
...
`;
  ...
  .map<ChatCompletionMessageParam>((message) => ({ role: message.user?.id === existingAgent.id ? "assistant" : "user", content: message.text || "", }));
  
  const GPTResponse = await openaiClient.chat.completions.create({ 
    messages: [ 
      { role: "system", content: instructions }, ...previousMessages, { role: "user", content: text }, ], model: "gpt-4o", }); 
